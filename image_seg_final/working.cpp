...
int Ford_Fulkerson (Graph& graph, int source, int sink, int width, int height )
{
    ...
    for ( int i=1; i<= imageSize; ++i )
    {
        ...
        if(rownum != 1 && colnum != 1 && rownum != height && colnum != width){
            ...
            if(weight1>threshold)
                g->addEdge(i,i+1,weight1);
            if(weight2>threshold)
                g->addEdge(i,i-1,weight2);
            if(weight3>threshold)
                g->addEdge(i,i+width,weight3);
            if(weight4>threshold)
                g->addEdge(i,i-width,weight4);
		}
        ////////////////////////////////////////////////////////////////////
        // first row
        /*if ( rownum == 1 )
...
int main(int argc, const char * argv[]) {
    ...
    std::string imageFile = baselocation + std::string("cars.jpg");
    ...
    Graph* inputGraph = createGraph(image.cols,image.rows,2,likelihoodArray);
